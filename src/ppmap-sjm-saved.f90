...
! Read in the observational images and bad pixel masks.
	    ...
	    pixset(lam) = cdelt2*3600.
            crota2 = 0.
            wavelengths(lam) = wl
            print *,'Peak SNR at',wavelengths(lam),' microns =',maxval(a)/sig, nx, ny
            do j = 1,ny
            do i = 1,nx
...
! Calculate system matrix, B. Also, set up data and noise vectors.
	    ...
	    mag = pixel/pixset(lam)
            ...
            isize = ihim-ilom+1
            jsize = jhim-jlom+1
            allocate(temparray(isize,jsize))  
            do j = 1,ny
               do i = 1,nx
                  if (i>ilom .and. i <= ihim+1 .and. &
                     ...
                     temparray(i-ilom+1,j-jlom+1) = obsset(lam,i,j) 
            ...
            enddo
            print*, lam
            write(tempfile,'(a,i1,a)') 'data',lam,'.fits'
            call writeimage2d(tempfile,temparray,isize,jsize,'','','',1,1,1,1,1,1,0,status)
	    ...
	    do kt = 1,Nt
                ...
                enddo
                enddo
!!$                write(tempfile,'(a,i1,a,i1,a)') 'refmodel',kb,'_',kt,'.fits'
!!$                call writeimage2d(tempfile,grid,nref,nref,'','','',1,1,1,1,1,1,0,status)
                grid = grid/mag**2
                iref = nref/2 + 1
...
! Set optical depth increment.
        ...
        B = B*dtau
        starting = .true.
        call writeimage2d('Bmatrix.fits',B,nobs,nstates,'','','',1,1,1,1,1,1,0,status)
...
! Impose a Gaussian prior on beta if there are multiple possible values.
        ...
        do while(starting)
           ...
           do while (t < tmax .and. it < maxiterations .and. &
              ...
              if ( (rchisqprev - rchisq)/rchisq < rchisqconv) then
                 write(*,'(a,i6,a,es11.4,es11.4,a,es9.2)') 'converged step',it, ' ; rchisq ', &
                      rchisq, rchisqprev, ' fractional delta ', (rchisqprev - rchisq)/rchisq
                 converged = .true.
                 starting = .false. 
              end if

              
              if ( .false. .and. (rchisq > rchisqprev) ) then
                 write(*,'(a,i6,a,es8.1,es8.1,a)') 'step ',it, 'rchi2, drchi2 ', rchisq, rchisq-rchisqprev, ' - diverged'
...
!!$                 ! set step back 
!!$                 rescale = 1./1.2**2
!!$                 epsilon = epsilon*rescale
!!$                 dtau = dtau*rescale
!!$                 B = B*rescale
!!$                 rho = rho/rescale
!!$                 print *,'Diverging at iteration',it,' decrease step size to',epsilon
!!$                 diverged = .false. ! this means it can step back as many times as necessary
        ...
        if(.not. t<tmax ) print*, 'Finished: tmax exceeded', t
        if(.not. it<maxiterations ) print*, 'Finished: maxiterations exceeded', it
